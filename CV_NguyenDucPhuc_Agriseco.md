# CURRICULUM VITAE - NGUYỄN ĐỨC PHÚC
**TRƯỞNG NHÓM PHÁT TRIỂN PHẦN MỀM**

![Nguyễn Đức Phúc Avatar](cv-avatar-pnd.jpg)

* **Địa chỉ:** Hà Nội, Việt Nam  
* **Điện thoại:** 0364.252.470  
* **Email:** phucnd.work@gmail.com  
* **Ngày sinh:** 06/04/1991 (Nam)  
* **Thể chất:** Chiều cao: 180 cm | Cân nặng: 80 kg (Sức khỏe tốt)  

---

## TÓM TẮT NĂNG LỰC CHUYÊN MÔN

* **Kinh nghiệm làm việc:** Lập trình viên có hơn 10 năm kinh nghiệm tham gia phân tích, thiết kế, phát triển và triển khai các ứng dụng phần mềm trên nền tảng Mobile và Web.
* **Kinh nghiệm nghiệp vụ:** Đã từng tham gia phát triển các sản phẩm trong lĩnh vực Tài chính, Ngân hàng và Bảo hiểm như: Ứng dụng chuyển tiền (FinBlue UK-VN), Module xác thực eKYC cho Ngân hàng, Ứng dụng quản lý bảo hiểm (Tập đoàn Bảo Việt) kết nối CSDL tập trung.
* **Cơ sở dữ liệu:** Thành thạo các hệ cơ sở dữ liệu chuyên dụng cho Mobile (Realm DB, SQLite, CoreData); có kiến thức cơ bản về các hệ cơ sở dữ liệu quan hệ (Oracle SQL, PostgreSQL, MySQL).
* **Quy trình & Công cụ:** Hiểu rõ quy trình phát triển phần mềm SDLC (Agile/Scrum), quản lý mã nguồn Git, thực hiện kiểm thử Unit Test, có kinh nghiệm sử dụng Docker và thiết lập chuỗi CI/CD tự động.
* **Làm việc nhóm & Quản lý:** Có khả năng làm việc cùng BA/QA/PM để làm rõ bài toán nghiệp vụ; từng tham gia đào tạo, hướng dẫn kỹ thuật (mentor) và quản lý tiến độ cho các nhóm lập trình viên.

---

## GIẢI THƯỞNG & THÀNH TỰU CÁ NHÂN

* **Nhân Viên Xuất Sắc Năm 2021** — Công ty TEADAO *(Ghi nhận đóng góp trong các dự án FinTech)*.
* **Mentor Xuất Sắc Năm 2020** — Công ty Rikkeisoft *(Ghi nhận đóng góp trong việc hỗ trợ đào tạo và hướng dẫn thành viên mới)*.
* **Nhân Viên Xuất Sắc Năm 2018** — Công ty Rikkeisoft *(Ghi nhận trách nhiệm và hiệu quả công việc trong năm)*.
* **Nhân Viên Mới Xuất Sắc Năm 2015** — Công ty Rikkeisoft *(Ghi nhận sự chủ động học hỏi và hoàn thành tốt nhiệm vụ được giao)*.

---

## KỸ NĂNG KỸ THUẬT CỐT LÕI

* **Nghiệp Vụ & Kiến Trúc:** Tài Chính - Ngân Hàng, Bảo Hiểm, FinTech, System Architecture, Clean Architecture, MVVM, SDLC, Agile / Scrum.
* **Cơ Sở Dữ Liệu:** Realm DB, SQLite, CoreData, Oracle SQL (Cơ bản), PostgreSQL (Cơ bản), MySQL (Cơ bản).
* **Ngôn Ngữ & Nền Tảng:** Swift, SwiftUI, Objective-C, Flutter (Dart), Kotlin, Java, RxSwift, RESTful APIs, WebSockets.
* **DevOps, Tools & Kiểm Thử:** Docker, CI/CD (GitHub Actions, Fastlane, Jenkins), Git, Unit Testing, Firebase, Tài liệu kỹ thuật.

---

## KINH NGHIỆM LÀM VIỆC CHI TIẾT

### **Neos Vietnam International Co., Ltd**
*Vị trí:* **iOS Technical Leader** | *Thời gian:* 03/2025 – Hiện tại
* **Phát triển ứng dụng:** Đảm nhận vai trò trưởng nhóm kỹ thuật phát triển ứng dụng *Calomama Plus* trên nền tảng SwiftUI và mô hình MVVM.
* **Phối hợp nghiệp vụ:** Phối hợp cùng BA và Backend team để làm rõ yêu cầu chức năng, thống nhất định dạng API và cấu trúc dữ liệu.
* **Quản lý chất lượng:** Thực hiện kiểm tra mã nguồn (Code review), hỗ trợ viết Unit Test, quản lý mã nguồn trên Git và chuẩn bị bản đóng gói ứng dụng.
* **Quản lý công việc:** Ước lượng thời gian hoàn thành (estimation), chia nhỏ công việc trong Sprint và hỗ trợ thành viên trong nhóm giải quyết vướng mắc kỹ thuật.

---

### **FPT Software**
*Vị trí:* **Senior Mobile Developer / Software Engineer** | *Thời gian:* 08/2023 – 03/2025
* **Phát triển chức năng:** Lập trình các module chức năng cho dự án phần mềm theo đúng tài liệu thiết kế và tiêu chuẩn của công ty.
* **Quy trình & Kiểm thử:** Thực hiện kiểm thử đơn vị (Unit Test), phối hợp cùng bộ phận QA/BA trong giai đoạn kiểm thử tích hợp và UAT.
* **Hỗ trợ đóng gói:** Tham gia cấu hình và duy trì luồng đóng gói tự động CI/CD trên Git.

---

### **Kinh Nghiệm Khác (2014 – 2023)**
* **Rikkeisoft, TEADAO, FinBlue, Tập đoàn Bảo Việt:** Từng giữ vị trí Team Leader / Senior Developer tại Rikkeisoft, TEADAO, FinBlue, Tập đoàn Bảo Việt (phát triển các sản phẩm FinTech, eKYC Ngân hàng, Chat doanh nghiệp, Bảo hiểm và đào tạo nhân sự).

---

## DỰ ÁN TIÊU BIỂU

| Tên Dự Án | Lĩnh Vực & Vai Trò | Công Nghệ Sử Dụng |
| :--- | :--- | :--- |
| **Bảo Hiểm Bảo Việt** | Bảo Hiểm / Tài Chính<br>*Software Engineer* | Swift, Alamofire, CoreData, REST API |
| **FinBlue Remittance** | FinTech / Chuyển tiền<br>*Senior Developer* | Flutter, Dart, SQLite, REST API |
| **Banking eKYC Module** | Bảo mật / Ngân hàng<br>*Technical Lead* | Flutter, iOS/Android, AI OCR, Live Video |
| **IM Chat Platform** | Truyền thông Doanh nghiệp<br>*Technical Lead* | iOS Native, WebSockets, E2EE, CallKit |
| **Inploi Recruitment** | Tuyển dụng / E-Commerce<br>*Project Leader* | Swift, Realm DB, Stripe API, APNS Push |

---

## HỌC VẤN & GIẢI THƯỞNG KHÁC

* **Đại Học FPT (FPT University)** — Kỹ Sư Phần Mềm *(Tốt nghiệp năm 2014)*
  * Tốt nghiệp chuyên ngành Kỹ thuật Phần mềm. GPA: 7.0 / 10.
* **Hackathon Windows 8 (2013):** Thành viên đội thi tham gia Vòng Chung kết.

---

## THÔNG TIN BỔ SUNG

* **Ngoại ngữ:** Tiếng Anh (Đọc hiểu tài liệu kỹ thuật, giao tiếp công việc).
* **Tác phong làm việc:** Tinh thần trách nhiệm cao, chủ động trong công việc, hòa đồng và sẵn sàng hỗ trợ đồng nghiệp.
